...
def fetch_moe_data():
    ...
    try:
        ...
        # 2. تنظيف البيانات لـ ast.literal_eval (أقوى في تحليل صيغ JS غير القياسية)
        print("🔧 جاري محاولة تحليل البيانات باستخدام AST...")
        
        # إزالة فواصل الأسطر والمسافات الزائدة
        js_data_text = js_data_text.replace('\n', '').replace('\t', '')
        
        # استبدال الـ quotes المفردة والمزدوجة (لضمان نجاح التقييم)
        js_data_text = js_data_text.replace("'", '"')
        
        # اقتباس المفاتيح غير المقتبسة: تحويل {key: "value"} إلى {"key": "value"}
        js_data_text = re.sub(r'([{\s,])\s*([a-zA-Z0-9_]+)\s*:', r'\1"\2":', js_data_text)
        
        # اقتباس القيم النصية التي قد تكون غير مقتبسة في ملفات JS
        # (هذه هي النقطة الحرجة التي تسبب invalid syntax)
        # النمط: بعد نقطتين : + مسافات + قيمة نصية (لا تحتوي على فواصل أو أقواس)
        js_data_text = re.sub(r'("\w+":\s*)([^"\[\]\{\},]+)', 
                             lambda m: m.group(1) + f'"{m.group(2).strip()}"', 
                             js_data_text)
        
        # إزالة الفواصل الزائدة قبل الأقواس المغلقة
        js_data_text = re.sub(r',\s*\]', ']', js_data_text)
        
        # 3. التحويل باستخدام json (الذي أصبح الآن مدعوماً بعد تنظيف المفاتيح والقيم)
        data = json.loads(js_data_text)
        ...
        print(f"❌ فشل التحليل النهائي (JSONDecodeError أو SyntaxError): {e}")
        return []
