...
def get_current_links_from_js(js_url, target_grade):
    ...
    try:
        ...
        # ب. التأكد من اقتباس المفاتيح (مثل: stage, link) التي لم يتم اقتباسها في JS
        # هذا هو التصحيح الأهم للمشكلة.
        json_text = re.sub(r'([a-zA-Z0-9_]+)\s*:', r'"\1":', json_text)

        # ج. استبدال علامات الاقتباس الفردية (إذا وجدت) بالزوجية
        json_text = json_text.replace("'", '"')
        
        # د. إزالة الفواصل الزائدة في نهاية المصفوفة (والتي تسبب خطأ JSON)
        ...
        print(f"رسالة الخطأ: {e.msg} | سطر: {e.lineno}، عمود: {e.colno}")
        return []
