...
import requests
import re
import ast # 🚨 NEW: Library for safe evaluation of literal structures
# =================================================================
# الإعدادات العامة
# =================================================================
...
def get_current_links_from_js(js_url, target_grade):
    ...
    try:
        ...
        # أ. إزالة أي مسافات زائدة وفواصل أسطر جديدة
        js_data_text = js_data_text.replace('\n', '').replace('\t', '')
        
        # ب. تغيير علامات الاقتباس المزدوجة حول المفاتيح إلى اقتباس فردي
        # ast.literal_eval تتطلب أن تكون جميع السلاسل النصية مُقتبسة بعلامات اقتباس مفردة (single quotes)
        # ولكن بما أن الكود المصدري يستخدم علامات مزدوجة أحياناً، سنقوم بالتحويل الشامل.
        
        # ج. التأكد من اقتباس المفاتيح (مثل: stage, link) واقتباس القيم باستخدام علامات اقتباس مفردة
        # نستخدم regex لتحويل {key:"value"} إلى {'key':'value'}
        js_data_text = re.sub(r'([a-zA-Z0-9_]+)\s*:\s*"([^"]*)"', r"'\1': '\2'", js_data_text)
        
        # د. اقتباس المفاتيح التي لم يتم اقتباسها في JS
        js_data_text = re.sub(r'([a-zA-Z0-9_]+)\s*:', r"'\1':", js_data_text)
        
        # هـ. استبدال جميع علامات الاقتباس المزدوجة المتبقية بعلامات مفردة (لأغراض التوافق مع ast)
        js_data_text = js_data_text.replace('"', "'")
        
        # و. إزالة الفواصل الزائدة في نهاية المصفوفة
        js_data_text = re.sub(r',\s*\]', ']', js_data_text)
        
        # ----------------------------------------------------
        
        # 3. التحليل باستخدام ast.literal_eval (يحل مشكلة التنسيق غير القياسي)
        books_data = ast.literal_eval(js_data_text) # 🚨 الحل النهائي 🚨
    ...
    except SyntaxError as e: # 🚨 تغيير معالجة الخطأ
